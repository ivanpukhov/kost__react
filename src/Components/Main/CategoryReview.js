...
const Category = () => {
    ...
    const fetchEstablishments = async (categoryId) => {
        try {
            const response = await axios.get(`https://rev.greenman.kz/api/user/${localStorage.userId}/reviews?categoryId=${categoryId}`);
    ...
    return (
        <main className="main">
            <EstablishmentList
                title={`Ваши отзывы ${currentCategory}` || 'Заведения'} // Используем название категории или запасной вариант
